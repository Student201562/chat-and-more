...
public class ClientObject implements Runnable {
    ...
    public void run() {
        ...
        try {
            DataInputStream inputStream = new DataInputStream(client.getInputStream()); //поток для прослушивания сообщений от клиента
            ...
            while (true) {
                try {
                    message = inputStream.readUTF();    //принимает сообщение от клиента
