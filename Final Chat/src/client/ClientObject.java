...
class ClientObject implements Runnable {
    ...
    void sendMessage(String message) {
        ...
        try {
            ...
            output.flush(); //если не сделать, будет долго записывать данные в поток
        } catch (IOException e) {
    ...
    public void run() {
        try {
                ...
                DataInputStream inputStream = new DataInputStream(client.getInputStream()); //принимает сообщения от сервера
                log.info("Input stream got");

                sendMessage(name);  //отправляется серверу имя клиента
                String message;
